...
// File: contracts/HISVTUBERNFT.sol
...
contract HISVTUBERNFT is ERC1155, Ownable, AccessControl {
    ...
    function generateRandomNumber(uint256 amount) public view returns (uint) {
        ...
        uint randomNumber = (randomHash % 43) + 1;
        for (uint256 i = 0; i < 43; i++) {
            if (randomNumber > 45) {
                randomNumber = 1;
            }
            // Adjust if randomNumber is 17 or above (to skip 17)
            if (randomNumber >= 17) {
                randomNumber++;
            }
            // Adjust if randomNumber is 25 or above (to skip 25)
            if (randomNumber >= 25) {
                randomNumber++;
            }
    ...
    function withdraw(address to) public onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        emit withdrawComplete(amount, to);
    }

...
describe("FreeMint", function () {
  async function deployTokenFixture() {
    ...
    return { FreeMint, owner, nonOwner, recipient1, recipient2, PRICE };
  }

  describe("Mint function with payment", function () {
    it("should fail when payment amount is incorrect", async function () {
      const { FreeMint, recipient1, PRICE } = await loadFixture(deployTokenFixture);

      // Try to mint with incorrect payment
      await expect(FreeMint.write.mint(
  ...
  describe("BulkMint function (owner only, no payment)", function () {
    it("should allow owner to bulk mint without payment", async function () {
      ...
      // Verify balances
      expect(await FreeMint.read.balanceOf([recipient1.account.address])).to.equal(1n);
      expect(await FreeMint.read.balanceOf([recipient2.account.address])).to.equal(1n);
    });
  ...
  describe("Withdraw function", function () {
    it("should allow owner to withdraw accumulated payments", async function () {
      const { FreeMint, owner, recipient1, PRICE } = await loadFixture(deployTokenFixture);
      const publicClient = await hre.viem.getPublicClient();

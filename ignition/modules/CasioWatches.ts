// npx hardhat ignition deploy ./ignition/modules/CasioWatches.ts --network localhost
// npx hardhat ignition deploy ./ignition/modules/CasioWatches.ts --network sepolia --verify
// npx hardhat ignition deploy ./ignition/modules/CasioWatches.ts --network minato --verify
// npx hardhat ignition deploy ./ignition/modules/CasioWatches.ts --network soneium --verify
...
function validateInput(projectName: string, projectData: any) {
  ...
  if (projectData.metadataEntries != 8) {
    throw new Error(`Project ${projectName} is not Casio Watches`);
  }
}
  // Verify inputs for this class of projects
const CasioWatchesModule = buildModule("CasioWatchesModule", (m) => {
  ...
  const CasioWatches = m.contract("CasioWatches", [
    name,
    symbol,
    baseURI,
    baseExtension,
...
export default CasioWatchesModule;